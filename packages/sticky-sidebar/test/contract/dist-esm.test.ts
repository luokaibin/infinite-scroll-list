...
describe('ESM 产物（npm 主通道）', () => {
  it('Node/SSR 导入为 no-op：不抛错、不触碰浏览器 API', async () => {
    ...
    expect(typeof (globalThis as any).matchMedia).toBe('undefined');

    expect(m.version).toBe(pkg.version);
    expect(typeof m.StickySidebar).toBe('function');
    expect(typeof m.registerStickySidebar).toBe('function');
  });
});
